...
- main.py координирует все модули. Только он вызывает executor — UI-модули не обращаются к нему напрямую.
- Слои UI, Engine и Core изолированы. Каждый знает только то, что необходимо для его работы.
- Наблюдение работает в daemon-потоке, таймер — в основном. Они не блокируют друг друга.
- activity_monitor.py получает функции меню через колбэки, а не через прямой импорт.
- Все модули импортируют Ritual, RitualResult, ActionType из schema.py.
- executor._get_template() — для создания файлов, FileTemplates.get_template() — для вставки через меню помощи.
- Конвертирует RuleTemplate в Ritual через to_ritual(), связывая Engine и Core.
- Файлы, редактор и LRS используются только конкретными модулями. Остальные о них не знают.
- Поток идёт только сверху вниз: UI → Engine → Core → Внешние системы. Обратных связей нет.
